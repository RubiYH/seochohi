...
export default function EnterStudentCard(props) {
  const { StudentCard, setSC, setCGN } = props.data;
  ...
  return (
    <>
      ...
      <span className={styles.description} style={{ color: "#d32f2f" }}>
        학생증 카드는 재학생 인증 용도로만 사용되며 인증 완료 후 안전하게 삭제됩니다.
      </span>
            ...
            <>
              ...
              {errors && (
                <div className={styles.error_wrapper}>
                  {errors.maxNumber && <span>이미지는 1장만 업로드할 수 있습니다.</span>}
                  {errors.acceptType && <span>지원되지 않는 파일 형식입니다</span>}
                  {errors.maxFileSize && (
                    <span>파일 크키가 너무 큽니다. (최대 {settings.maxImageSize / 1000000}MB)</span>
                  )}
                </div>
              )}
