    ...
    getConnection((connection) => {
      connection.query(
        `SELECT * FROM \`users\` WHERE \`Grade\`=${connection.escape(
          data.grade
        )} AND \`Class\`=${connection.escape(
          data.class
        )} AND \`StudentID\`=${connection.escape(data.studentID)}`,
        (err, results, fields) => {
          if (err) {
            res.json({
              status: "error",
            ...
            if (results.length > 0) {
              res.status(200).json({
                ...
                message:
                  "이미 가입한 학생입니다. 본인이 가입한 적이 없다면 문의해주세요.",
              });
              return;
            } else {
              const ID = `${data.grade}${
                ...
                `${data.studentID}`.length === 1
                  ? `0${data.studentID}`
                  : `${data.studentID}`
              }`;

              connection.query(
                `SELECT * FROM \`pending_users\` WHERE \`ID\`=${connection.escape(
                  ID
                )}`,
                (err, results, fields) => {
                  if (err) {
                  ...
                  if (data.gender === "m") {
                    avatarurl =
                      "https://cdn-icons-png.flaticon.com/512/257/257634.png";
                  } else if (data.gender === "f") {
                    avatarurl =
                      "https://cdn-icons-png.flaticon.com/512/257/257651.png";
                  }
                  ...
                  connection.query(
                    `INSERT INTO \`pending_users\` (ID, Username, Grade, Class, StudentID, Gender, Email, Phone, Password, Salt, Session, AvatarURL, Attributes, Favorites, CreatedAt, Settings, Status) VALUES (${connection.escape(
                      ...
                      data.username.trim()
                    )}, ${connection.escape(data.grade)}, ${connection.escape(
                      data.class
                    )}, ${connection.escape(
                      data.studentID
                      ...
                      favorites
                    )},CURRENT_TIMESTAMP(), ${connection.escape(
                    ...
                    (err, results, fields) => {
                      if (err) {
                        ...
                        console.log(
                          `${ID} ${data.username} : 회원가입 완료: 승인 대기 중.`
                        );
