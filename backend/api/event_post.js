      ...
      getConnection((connection) => {
        connection.query(`SELECT * FROM \`count\``, (err, results, fields) => {
          if (err) {
            ...
            let PostID =
              results[0]["Event_PostCount"] === null
                ? 1
                : parseInt(results[0]["Event_PostCount"]) + 1;
                  ...
                  if (err) {
                    ...
                    if (images.length > 0) {
                      images.forEach((i) => {
                        ...
                        //이미지 저장
                        fs.writeFile(
                          `./files/storage/Events/Notes/${PostID}_${userID}_${cl}` +
                            extension,
                          imgFromBase64,
                          function (err) {
                            if (err) {
                              ...
                              if (
                                ...
                                imageURLs.push(
                                  ...
                                  }/static/events/notes/${PostID}_${userID}_${
                                    c + 1
                                  }${extension}`
                                );
                                //finished
      ...
      function postNew(PostID, userID, username, AvatarURL, title, images) {
        ...
        getConnection((connection) => {
          connection.query(
            ...
            )}, CURRENT_TIMESTAMP(), '[0,[]]', 0, '[]', ${connection.escape(
              JSON.stringify(Tags)
            )})`,
            (err, results, fields) => {
              if (err) {
                ...
                getConnection((connection) => {
                  connection.query(
                    `UPDATE \`count\` SET \`Event_PostCount\`=${connection.escape(
                      PostID
                    )}`
                  );
                });
