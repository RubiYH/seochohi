...
module.exports = function (app) {
  app.get("/api/handleviews", authJWT, async function (req, res) {
    ...
    getConnection((connection) => {
      connection.query(
        `SELECT \`Views\` FROM ${connection.escape(table)} WHERE \`PostID\`=${connection.escape(
          pid
        ...
        (err, results, fields) => {
          ...
          //이미 조회했을면
          if (ViewedList.some((v) => v.userID === userID && v.username === username)) {
            res.json({
          ...
          let newList = [
            ...
            [...JSON.parse(results[0].Views)[1], { userID: userID, username: username }],
          ];

          connection.query(
            `UPDATE \`${connection.escape(table)}\` SET \`Views\`=${connection.escape(
