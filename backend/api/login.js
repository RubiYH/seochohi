  ...
  app.post("/api/login", getIP, async function (req, res) {
    ...
    if (
      ...
      getConnection((connection) => {
        connection.query(
          `SELECT \`Password\`, \`Salt\` FROM \`users\` WHERE \`ID\`=${connection.escape(
            ID
          )}`,
          (err, results, field) => {
            if (err) {
              ...
              if (results?.length > 0) {
                ...
                //입력한 비밀번호를 바탕으로 암호화
                const [getSalt, getEncrypted] = encryptText(
                  data.password,
                  salt
                );
                  ...
                  connection.query(
                    ...
                    (err, results, field) => {
                      ...
                      if (
                        ...
                        console.log(
                          `[${req.getIP}] ${userID} ${username} : 로그인 완료`
                        );
                      }
                    }
                  );
