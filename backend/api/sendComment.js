            ...
            connection.query(
              ...
              (err, results, fields) => {
                ...
                const newComment = {
                  ...
                  commentID: Number(
                    `${data.postID}${data.postUserID}${new Date().getTime()}`
                  ),
                ...
                const mergedComments = [...getComments, newComment].sort(
                  (a, b) => {
                ...
                connection.query(
                  `UPDATE \`${getTable(
                    data.table
                  )}\` SET \`Comments\`=${connection.escape(
                  ...
                  (err, results, fields) => {
                    if (err || results.affectedRows < 1) {
                      ...
                      connection.query(
                        `UPDATE \`${getTable(
                          data.table
                        )}\` SET \`total_comments\`=${connection.escape(
                          total_comments
                        )} WHERE \`PostID\`=${connection.escape(
                          data.postID
                        )} AND \`UserID\`=${connection.escape(
                          data.postUserID
                        )}`,
                        (err, results, fields) => {
                          if (err || results.affectedRows < 1) {
